import React from 'react';

const Terms = () => {
return (
<div style={{ padding: '2rem', fontFamily: 'Arial, sans-serif' }}>
<h1 style={{ textAlign: 'center' }}>Terms and Conditions</h1>
Effective Date: [20/11/2024]

  <h2>1. Introduction</h2>
  <p>
    Welcome to <strong>TaskCoin</strong> ("the Site"). The Site is an experimental platform created for
    educational purposes only. These Terms and Conditions ("Terms") govern your access and use of theba Site.
    By using the Site, you agree to these Terms in full. If you do not agree, please discontinue use immediately.
  </p>

  <h2>2. Experimental and Educational Purpose</h2>
  <p>
    This Site has been developed as part of an educational project to demonstrate web development and task-based
    user interaction models. <strong>All features, functionalities, and representations, including but not limited
    to monetary symbols (₹ or any other currency signs), are for educational and demonstration purposes only.</strong>
  </p>

  <h3>Important Disclaimer:</h3>
  <ul>
    <li><strong>No Real Payments:</strong> The Site does not and will not provide any real monetary compensation or rewards. Any amounts or figures displayed are purely for illustrative and educational purposes.</li>
    <li><strong>No Financial Transactions:</strong> The Site does not facilitate any actual financial transactions or payouts.</li>
    <li><strong>Simulation Only:</strong> All tasks, rewards, and activities on the Site are part of a simulated environment meant to showcase technical capabilities.</li>
  </ul>

  <h2>3. User Conduct</h2>
  <p>
    By using the Site, you agree to:
  </p>
  <ul>
    <li>Use the Site responsibly and for its intended educational purpose.</li>
    <li>Refrain from attempting to exploit, manipulate, or misuse the Site's functionality.</li>
    <li>Understand that all representations of "earnings" or "rewards" are fictional and for demonstration purposes only.</li>
  </ul>

  <h2>4. Intellectual Property</h2>
  <p>
    All content on the Site, including text, graphics, and code, is the intellectual property of <strong>TaskCoin</strong> or its creators, unless otherwise stated. This content is provided solely for educational use and may not be copied, reproduced, or distributed for commercial purposes.
  </p>

  <h2>5. No Warranties</h2>
  <p>
    The Site is provided "as-is" without any warranties of any kind. We make no representations or guarantees regarding:
  </p>
  <ul>
    <li>The accuracy or completeness of the content.</li>
    <li>The availability or performance of the Site.</li>
    <li>The security or reliability of the Site.</li>
  </ul>

  <h2>6. Limitation of Liability</h2>
  <p>
    To the maximum extent permitted by law, the creators and operators of the Site will not be liable for any damages, losses, or claims arising out of your use of the Site. This includes, but is not limited to, any misunderstandings about potential earnings, technical issues, or reliance on the Site's content.
  </p>

  <h2>7. Changes to These Terms</h2>
  <p>
    We reserve the right to modify or update these Terms at any time. Any changes will be effective immediately upon posting. It is your responsibility to review these Terms periodically for updates.
  </p>

  <h2>8. Age Restriction and Consent</h2>
  <p>
    Users of <strong>TaskCoin</strong> must be at least 18 years old to access and use the Site. If you are under 18 years of age, you are strictly prohibited from using this application. By using the Site, you confirm that you meet this age requirement.
  </p>
  <p>
    Furthermore, users should be aware that tasks on the Site may include or link to content that contains adult advertisements. By proceeding to use the Site, you acknowledge and consent to the possibility of encountering such content as part of the simulated tasks. If you are not comfortable with this, you are advised to refrain from using the Site.
  </p>
  <p>
    It is the user's responsibility to ensure they are compliant with this policy. The creators of the Site disclaim any liability for exposure to adult content due to non-compliance with the age restriction or lack of consent.
  </p>



  <p><strong>Thank you for understanding that this Site is an educational experiment and not a commercial platform. Enjoy exploring!</strong></p>
</div>

);
};

export default Terms;